...
public interface ReportRepository extends JpaRepository<Order, String> {
    @Query("""
                SELECT new com.NamVu.dto.response.report.OrderSummary(
    ...
    @Query("""
                SELECT new com.NamVu.dto.response.report.WeeklyRevenueTrend(
    ...
    @Query("""
                SELECT new com.NamVu.dto.response.report.RevenueByOrderType(
    ...
    @Query("""
                SELECT new com.NamVu.dto.response.report.CategoryReport(
                    c.name,
    ...
    @Query("""
                SELECT new com.NamVu.dto.response.report.SupplierReport(
                    s.name,
                    COALESCE(SUM(od.quantity), 0),
                    COALESCE(SUM(od.priceAtPurchase * od.quantity), 0)
                )
    ...
    @Query("""
                SELECT new com.NamVu.dto.response.report.UserGrowthRate(
    ...
    @Query("""
            SELECT new com.NamVu.dto.response.report.ProductReport(
                p.code, p.name,
                COALESCE(SUM(od.quantity), 0),
                COALESCE(SUM(od.priceAtPurchase * od.quantity), 0)
            )
    ...
    @Query("""
            SELECT new com.NamVu.dto.response.report.RevenueByProduct(
                p.code,
